...
一般Windows Server 2016会提供4.6 .请务必安装到4.7及以上版本.以便后续升级
...
[下载地址](https://pan.opo8.com/部署/融合平台部署/Redis.zip)
...
![](https://xindoo-doc.oss-cn-shanghai.aliyuncs.com/doc20191104155722.png)

在IIS中创建一个网站 命名为 `CASTLENEW.WEB

网站指向根目录 为 这些文件所在的目录

端口请使用一个未被占用的端口, 此处建议6679

使用文本编缉器(非notepad)打开login.html

![](https://xindoo-doc.oss-cn-shanghai.aliyuncs.com/doc20191104160226.png)

大约在135行左右

这个地址是刚才创建的后台API的地址. 改成你实际使用的后台API地址端口

~~~javascript
localStorage.setItem('baseUrl','http://xd-iot.com:6678/');
~~~

大约在139行左右

这个地址是刚才创建的图片服务器API的地址,改成你实际部署的图片服务器的地址端口

~~~javascript
localStorage.setItem("temp",'http://xd-iot.com:6680/');
~~~

通过web部署的 6679端口打开网页 . 进入/login.html
...
2. login.html中的API地址错误.
3. 网络环境是外网没有把API端口映射到外网
4. 缓存没有清空. 请清空浏览器缓存后登录

如果登录成功但是看不见地图.则说明可能的几个原因

1. 未添加地图
2. 地图文件丢失
3. 图片服务器端口错误或者未部署成功
4. 网络环境是外网没有把图片服务器端口映射到外网
